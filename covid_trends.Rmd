...
setwd("C:\\Users\\Abe\\Desktop\\Dataquest modules\\Data Analyst in R\\Guided Projects\\project_1")
...
For documentation purposes later, we will be storing the column names into a character vector. The structure of the object is verified as below.
...
Looking further into the dataset, we can see that the dataframe has nearly 11,000 rows and 14 columns. We can also see that data were collected on a daily basis and were labeled per continent all the way to the province/state level. We can also see that the dataframe contains data for all states and data for each state (depending on which country is situated).
...
Since our analysis focuses on data collected per country, we will be filtering out rows with the province/state column to be "All states" (since that contains the aggregated data per country). Afterwards, we will create another dataframe which includes only the columns relevant to the analysis. Since the dataframe also contains cumulative data, we will be excluding those variables from the analysis since our focus will be on the daily numbers.

```{r filter_df, include=TRUE}

covid_df_all_states <- covid_df %>% filter(Province_State ==  "All States") %>% select(-Province_State) ##we can remove province_state since we have already filtered out our data to all states

covid_df_all_states_daily <- covid_df_all_states %>% select(Date, Country_Region, active, hospitalizedCurr, daily_tested, daily_positive) ##Select columns related to daily measures
...
covid_df_all_states_daily_sum <- covid_df_all_states_daily %>% group_by(Country_Region) %>% summarize(tested = sum(daily_tested), positive = sum(daily_positive), active = sum(active),hospitalized = sum(hospitalizedCurr)) %>% arrange(desc(tested)) ##Aggregate daily data per country
...
positive_tested_top_3 <- round(sort(positive_cases/tested_cases, decreasing = TRUE),4)[1:3]
...
Based on the methods above, we have identified three countries with the highest positivity/case ratios namely, United Kingdom, United States, and Turkey. Although United Kingdom had the highest positivity ratio, the United States remains to have the highest number of cases accumulated throughout the period, with over 10 times compared to the two countries mentioned.
